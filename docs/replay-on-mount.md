...
A collection can unmount while the app-wide broker keeps receiving events. Those events are stored in `EventLogStore`, so the collection can later converge from local history instead of always fetching a fresh snapshot.

## Freshness state

Each collection key `(modelName, scope)` has a base watermark: the newest `syncId` through which that collection has handled all relevant signals.
...
1. no watermark → `Snapshot`;
2. resync newer than watermark → `Snapshot`;
3. watermark at or ahead of cursor → no replay;
4. prune floor above watermark → `Snapshot`;
5. otherwise → replay model events after the watermark.
...
The broker periodically applies configured retention caps:
...
If pruning removes any part of a collection's missing range, replay is unsafe and the broker emits `Snapshot`.

## Watermark batching

`markApplied` updates pending watermarks in memory and flushes them durably on an interval (100 ms by default) and on broker shutdown. A stale durable watermark may cause a few events to replay after a crash, but synced upserts and deletes are idempotent.

Configure the interval with `broker.watermarkFlushEvery`.
