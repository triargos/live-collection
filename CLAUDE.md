...
2. **Draft the interface as concise TypeScript pseudocode — no implementation bodies.** Show:
   - the `Effect.Service` / `Context.Tag` interface: method signatures with typed errors in the channel,
...
4. **Tests against the locked interface**, then **implement to green** in vertical slices.

**Do not write implementation before the interface design is locked and tests exist against it.** When a design turns out wrong, rewind and redesign — don't polish a flawed shape.
...
6. **`core` exposes Effect service interfaces (tags); implementations are layers.** This is the Effect-idiomatic expression of the spec's "the X is the seam" properties and keeps the DAG acyclic.
